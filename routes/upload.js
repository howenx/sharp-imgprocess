...
router.post('/upload', function(req, res, next) {
    ...
    try {
        if (minify_flag != 'undefined' && req.body.params) {
            ...
            if (mimetype === 'image/png' || mimetype === 'image/jpeg' || mimetype === 'image/gif' || mimetype === 'image/webp') {
                fs.readFile(req.files.displayImage.path, function(err, data) {
                    if (!req.files.displayImage.name) {
                        console.log(colors.gray("The image file transfer has an error."));
                        res.jsonp({
                            error: '444',
                        ...
                        fs.writeFile(fullPath, data, function(err) {
                            if (err) {
                                console.log(colors.gray("Write fullsize image found an error."));
                                res.jsonp({
                                    error: '445',
                                    message: 'Write fullsize image found an error.'
                                });
                            } else {
                                console.log(colors.gray('Write fullsize image success.'));
                                if (mimetype === 'image/png' || mimetype === 'image/jpeg') {
                                    ...
                                    if (imageSize >= 1048576 && minify_flag === 'minify') {
                                        ...
                                        fs.writeFile(minifyPath, data, function(err) {
                                            if (err) {
                                                console.log(colors.gray("Copy minify image found an error."));
                                                res.jsonp({
                                                    error: '445',
                                                ...
                                                console.log(colors.gray("Image Uploaded, But it's size less than 1M and no compression."));
                                                var compress = {
                                                    error: 'ok',
                                        ...
                                        fs.writeFile(minifyPath, data, function(err) {
                                            if (err) {
                                                console.log(colors.gray("Unminify upload images found an error."));
                                                res.jsonp({
                                                    error: '445',
                                                ...
                                                console.log(colors.gray('Unminify upload images success.'));
                                                var compress = {
                                                    error: 'ok',
                                                    before: (imageSize / 1048576).toFixed(2) + "M",
                                                    after: (imageSize / 1048576).toFixed(2) + "M",
                                ...
                                //file mismatch return fullsize url
                                    ...
                                    console.log(colors.gray("Image Uploaded, But No Compression.The image type can't be compressed."));
                                    var compress = {
                                        error: 'ok',
                                        before: (imageSize / 1048576).toFixed(2) + "M",
                                        after: (imageSize / 1048576).toFixed(2) + "M",
                ...
                console.log(colors.gray("Image type mismatch."));
                res.jsonp({
                    error: '445',
            ...
            console.log(colors.gray("No minify flag parameter."));
            res.jsonp({
                error: '445',
    ...
    try {
        ...
        if (file.match(/[^\/]+(\.(jpg|jpeg|JPG|JPEG|png|PNG|gif|GIF|webp|WEBP))$/) != null && typeof file.match(/[^\/]+(\.(jpg|jpeg|JPG|JPEG|png|PNG|gif|GIF|webp|WEBP))$/) != 'undefined') {
            ...
            if (file.match(/[\d]+×[\d]+/g) != null && typeof file.match(/[\d]+×[\d]+/g) != 'undefined') {
                ...
                if (file.match(/[a-zA-Z0-9]+\.(jpg|jpeg|JPG|JPEG|png|PNG|gif|GIF|webp|WEBP)/) != null && typeof file.match(/[a-zA-Z0-9]+\.(jpg|jpeg|JPG|JPEG|png|PNG|gif|GIF|webp|WEBP)/) != 'undefined') {
                    ...
                    if (file_nm.match(/(\.(jpg|jpeg|JPG|JPEG|png|PNG|gif|GIF|webp|WEBP))$/) != null && file_nm.match(/(.(jpg|jpeg|JPG|JPEG|png|PNG|gif|GIF|webp|WEBP))$/) != 'undefined') {
                        ...
                        if (type_origin === 'jpg' || type_origin === 'jpeg' || type_origin === 'png') {
                            ...
                            console.log(colors.gray("The original file can't be thumb because of the type."));
                            if (match != null && typeof match != 'undefined') {
                                res.sendFile(process.cwd() + '/uploads/fullsize/' + file_nm);
                            } else {
                                res.jsonp({
                        ...
                        console.log(colors.gray("The original image type not match the require."));
                        if (match != null && typeof match != 'undefined') {
                            res.sendFile(process.cwd() + '/uploads/fullsize/' + file_nm);
                        } else {
                            res.jsonp({
                    ...
                    console.log(colors.gray("The thumbnail namme not match the require."));
                    if (match != null && typeof match != 'undefined') {
                        res.sendFile(process.cwd() + '/uploads/fullsize/' + file_nm);
                    } else {
                        res.jsonp({
                ...
                console.log(colors.gray("Please assign the width and height."));
                if (match != null && typeof match != 'undefined') {
                    res.sendFile(process.cwd() + '/uploads/fullsize/' + file_nm);
                } else {
                    res.jsonp({
            ...
            console.log(colors.gray("Thumb image type mismatch."));
            if (match != null && typeof match != 'undefined') {
                res.sendFile(process.cwd() + '/uploads/fullsize/' + file_nm);
            } else {
                res.jsonp({
        ...
        if (file_match != null && typeof file_match != 'undefined') {
            ...
            if (match != null && typeof match != 'undefined') {
                ...
                fs.lstat(localpath, function(err, stats) {
                    //if not exist,404.
                    if (err) {
                        console.log(colors.gray('the request file not found.'));
                        next();
                    } else {
                ...
                console.log(colors.gray('Not match the request path.'));
                next();
            }
        } else {
            console.log(colors.gray('File type not match.'));
            next();
        }
    } catch (e) {
        console.log(colors.gray('catch exception:' + e));
        next(e);
    }
});
