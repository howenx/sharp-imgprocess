...
function png_minify(input, output_dic, output_nm,callback) {
    ...
    .run(function(err, files) {
        ...
        console.log('> '+colors.grey('Time: '+dateformat(new Date(), 'yyyy-mm-dd HH:MM:ss TT'))+'\t'+colors.yellow('压缩前图片大小：' + (input.length / 1048576).toFixed(2) + "M" + "  压缩后图片大小：" + (files[0].contents.length / 1048576).toFixed(2) + "M" + " 压缩比：" + ((1 - files[0].contents.length / input.length) * 100).toFixed(2) + "%"));
...
exports.minify = function(directory, file, output_dic, output_nm,callback) {
    ...
    try {
        if (hash[ext]) {
            console.log('> '+colors.grey('Time: '+dateformat(new Date(), 'yyyy-mm-dd HH:MM:ss TT'))+'\t'+colors.yellow('压缩文件名：' + file));
